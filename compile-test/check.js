...
const error = (str) => {
  process.stdout.write(`${source.red('Error:')} ${str}\n`);
};

const ok = (str) => {
  process.stdout.write(` ${source.green(str)}\n`);
};

const failed = (str) => {
  process.stdout.write(` ${source.red(str)}\n`);
};
...
const downloadProject = (repo, folder) => {
  ok(`Cloning ${repo}`);
  fs__namespace.rmdirSync(folder, { recursive: true });
  if (!fs__namespace.existsSync(folder)) {
    fs__namespace.mkdirSync(folder);
  }
  child_process.execSync(`git clone ${repo}`, { cwd: folder });
};

const process$1 = async (options) => {
  const run = (command, test, folder) => {
    write(command);
    let log;
    try {
      const cwd = (folder) ? folder : options.projectFolder;
      log = child_process.execSync(command, { cwd: cwd });
      ok('Success');
      if (options.verbose) {
        write(log);
      }
  ...
  if (!fs__namespace.existsSync(options.projectFolder)) {
...
const prepareBuild = (options) => {
  ...
  const log = child_process.execSync('xcodebuild -workspace App.xcworkspace -scheme App -showdestinations', { cwd: folder });
...
const writeFailure = (options, command, log) => {
  ...
  fs__namespace.writeFileSync(filename, data);
};

const loadResults = () => {
  if (fs__namespace.existsSync(resultFileName)) {
    const json = fs__namespace.readFileSync(resultFileName);
...
const saveResults = (data) => {
  fs__namespace.writeFileSync(resultFileName, JSON.stringify(data, null, 2));
};

const writeHtml = () => {
    const results = JSON.parse(fs__namespace.readFileSync('./results/results.json'));
    ...
    fs__namespace.writeFileSync('./results/plugins.html', html);
};
...
const plugins = JSON.parse(fs__namespace.readFileSync('./plugins.json'));
